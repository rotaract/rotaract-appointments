...
* Introduced validation for Meilisearch configuration parameters in the admin module.

=== Changed

* Meilisearch-related scripts are now enqueued only if both *ROTARACT_MEILISEARCH_URL* and *ROTARACT_MEILISEARCH_API_KEY* are defined.
* Appointment owner field in the admin interface is disabled when Meilisearch parameters are missing.

=== Fixed

* Prevented errors and redundant script loading when Meilisearch integration is not configured.
